...
class Code:
    ...
    def from_http_status(cls, status: int) -> Code:
        _MAP: dict[int, Code] = {
            400: cls.INVALID_ARGUMENT,
            401: cls.UNAUTHENTICATED,
            403: cls.PERMISSION_DENIED,
            404: cls.NOT_FOUND,
            409: cls.ABORTED,
            429: cls.RESOURCE_EXHAUSTED,
            499: cls.CANCELED,
            500: cls.INTERNAL,
            501: cls.UNIMPLEMENTED,
            503: cls.UNAVAILABLE,
            504: cls.DEADLINE_EXCEEDED,
        }
...
class SpeconnError(Exception):
    ...
    def encode(self, fmt: str) -> bytes:
        from specodec import respond, SpecCodec, SpecWriter

        def _enc(w: SpecWriter, obj: SpeconnError) -> None:
            w.begin_object(2)
            w.write_field("code")
            w.write_string(obj.code.as_str())
    ...
    def decode(payload: bytes, fmt: str) -> SpeconnError:
        ...
        try:
            code, message = dispatch(
                SpecCodec(encode=lambda w, o: None, decode=_dec), payload, fmt
            )
