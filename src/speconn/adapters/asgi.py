...
from ..server_channel import ASGIServerChannel
...
def create_asgi_channel_adapter(router: SpeconnRouter):
    async def asgi_app(scope: dict, receive: Callable, send: Callable) -> None:
        if scope["type"] != "http":
            return
        ...
        channel = ASGIServerChannel(receive, send, path, headers, local_addr, remote_addr)
        await router.handle_channel(channel, path, headers, local_addr, remote_addr)
